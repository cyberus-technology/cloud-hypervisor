...
struct PciHotplugSharedState {
    ...
    // Let the bus keep weak refs while this shared state owns the strong refs.
...
pub struct DeviceManager {
    ...
    pci_hotplug_state: Arc<Mutex<PciHotplugSharedState>>,
...
/// MMIO-accessible controller for ACPI PCI hotplug register accesses.
...
impl AcpiPciHotplugController {
    ...
    fn eject_device(
...

        {
            ...
            shared_state.cleanup_vfio_container();
        }

        if let Some(virtio_device) = &virtio_device {
            for mapping in virtio_device.lock().unwrap().userspace_mappings() {
                // SAFETY: userspace_mappings only contains mappings created by the VMM.
                unsafe {
                    self.memory_manager
                        .lock()
                        .unwrap()
...
impl DeviceManager {
    fn pci_hotplug_state(&self) -> std::sync::MutexGuard<'_, PciHotplugSharedState> {
        self.pci_hotplug_state.lock().unwrap()
    }

    fn push_bus_device(&self, bus_device: Arc<dyn BusDeviceSync>) {
        self.pci_hotplug_state().push_bus_device(bus_device);
    }
    ...
    pub fn new(
        ...
        let pci_hotplug_state = Arc::new(Mutex::new(PciHotplugSharedState {
            pci_segments: pci_segments.clone(),
            selected_segment: 0,
            bus_devices: Vec::new(),
            vfio_container: None,
        ...
        let acpi_pci_hotplug_controller =
            Arc::clone(&device_manager.lock().unwrap()._acpi_pci_hotplug_controller);

        address_manager
            .mmio_bus
            .insert(
                acpi_pci_hotplug_controller as Arc<dyn BusDeviceSync>,
                acpi_address.0,
    ...
    fn add_pci_devices(&mut self) -> DeviceManagerResult<()> {
        ...
        let mut iommu_attached_devices = Vec::new();
        {
            ...
            if let Some(iommu_device) = iommu_device {
                ...
                self.pci_hotplug_state().iommu_attached_devices =
                    Some((dev_id, iommu_attached_devices));
            }
        }
        ...
        for segment in &self.pci_segments {
            ...
            if let Some(pci_config_io) = segment.pci_config_io.as_ref() {
                self.push_bus_device(Arc::clone(pci_config_io) as Arc<dyn BusDeviceSync>);
            }

            self.push_bus_device(Arc::clone(&segment.pci_config_mmio) as Arc<dyn BusDeviceSync>);
        }
    ...
    fn add_vfio_device(
        ...
        let shared_vfio_container = self.pci_hotplug_state().vfio_container.clone();
        let vfio_container = if device_cfg.iommu {
            ...
            vfio_container
        } else if let Some(vfio_container) = &shared_vfio_container {
            Arc::clone(vfio_container)
        } else {
            let vfio_container = self.create_vfio_container()?;
        ...
        if needs_dma_mapping {
            ...
            let virtio_mem_devices = self.pci_hotplug_state().virtio_mem_devices.clone();
            for virtio_mem_device in &virtio_mem_devices {
                virtio_mem_device
                    .lock()
                    .unwrap()
    ...
    fn add_pci_device(
        ...
        self.push_bus_device(Arc::clone(&bus_device));
    ...
    pub fn update_memory(&self, new_region: &Arc<GuestRegionMmap>) -> DeviceManagerResult<()> {
        ...
        if let Some(vfio_container) = &self.pci_hotplug_state().vfio_container {
            // vfio_dma_map is unsound and ought to be marked as unsafe
            #[allow(unused_unsafe)]
            // SAFETY: GuestMemoryMmap guarantees that region points
            // to len bytes of valid memory starting at as_ptr()
    ...
    pub fn add_device(
        ...
        self.pci_hotplug_state().pci_segments[device_cfg.pci_segment as usize].pci_devices_up |=
            1 << bdf.device();
    ...
    pub fn remove_device(&mut self, id: &str) -> DeviceManagerResult<()> {
        ...
        self.pci_hotplug_state().pci_segments[pci_segment_id as usize].pci_devices_down |=
    ...
    fn hotplug_virtio_pci_device(
        ...
        // Update the PCIU bitmap
        self.pci_hotplug_state().pci_segments[handle.pci_segment as usize].pci_devices_up |=
            1 << bdf.device();
    ...
    pub fn iommu_attached_devices(&self) -> Option<(PciBdf, Vec<PciBdf>)> {
        self.pci_hotplug_state().iommu_attached_devices.clone()
    }
