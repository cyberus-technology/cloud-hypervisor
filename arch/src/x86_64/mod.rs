...
pub use msr_filter::filter_denied_msrs;
...
pub enum Error {
    ...
    #[error("Unable to apply MSR filter: Bitmaps exceed maximum permitted memory usage")]
    MsrFilterTooLarge,
    ...
    if kvm_hyperv {
        ...
        for msr in msr_definitions::hyperv::HYPERV_MSRS {
            if permitted_msrs.remove(&msr) {
                trace!("Removed {msr:#x} from the set of supported MSRs");
            }
        }
    }
