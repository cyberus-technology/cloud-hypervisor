...
pub fn generate_profile_data(
    ...
    generate_msr_profile_data_with(
        MsrProfileDataParams {
            ...
            forbidden_hypervisor_msrs: &msr_definitions::kvm::PROFILE_UNPERMITTED_MSRS[..],
        },
        msr_data_file,
        msr_data_license_file,
    )
...
struct MsrProfileDataParams<'a, const N: usize> {
    ...
    /// A list of known hypervisor MSRS that should be forbidden. This list is
    /// only used to help us detect MSRs that we might not be aware of.
    forbidden_hypervisor_msrs: &'a [u32],
}
...
fn generate_msr_profile_data_with<'a, const N: usize>(
    ...
    for msr in supported_msrs.difference(&permitted_msrs) {
        ...
        let is_hypervisor_msr = forbidden_hypervisor_msrs.contains(msr);
        ...
        if is_proc_feat_msr || is_hypervisor_msr || is_architectural_msr || is_non_architectural_msr
        {
            continue;
        }
