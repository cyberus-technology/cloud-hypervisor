...
mod tests {
    ...
    #[cfg(feature = "kvm")]
    use crate::x86_64::cpu_profile::{CpuIdProfileData, MsrProfileData};
    ...
    #[test]
    fn cpu_profile_host_loads_no_data() {
        assert_eq!(CpuProfile::Host.cpuid_data(true), None);
        assert_eq!(CpuProfile::Host.cpuid_data(false), None);
        assert_eq!(CpuProfile::Host.msr_data(), None);
    }

    /// Check that the `CpuProfile::cpuid_data` and `CpuProfile::msr_data` methods
    /// coincide with direct deserialization for the `sapphire-rapids` profile.
    #[cfg(feature = "kvm")]
    #[test]
    fn cpu_profile_loading_sapphire_rapids() {
        // Now check that the methods coincide with direct deserialization. For the
        // Sapphire Rapids profile this should be the case when `amx` is enabled.
        let profile = CpuProfile::SapphireRapids;
        let cpuid_data = profile.cpuid_data(true).unwrap();
        let deserialized_cpuid_data: CpuIdProfileData =
            serde_json::from_slice(include_bytes!("./cpu_profiles/sapphire-rapids.cpuid.json"))
                .unwrap();

        assert_eq!(cpuid_data, deserialized_cpuid_data);

        let msr_data = profile.msr_data().unwrap();
        let deserialized_msr_data: MsrProfileData =
    ...
    /// Check that the `CpuProfile::cpuid_data` and `CpuProfile::msr_data` methods
    /// coincide with direct deserialization for the `skylake` profile.
    #[cfg(feature = "kvm")]
    #[test]
    fn cpu_profile_loading_skylake() {
        // Now check that the methods coincide with direct deserialization. For the
        // Sapphire Rapids profile this should be the case when `amx` is enabled.
        let profile = CpuProfile::Skylake;
        let cpuid_data = profile.cpuid_data(true).unwrap();
        let deserialized_cpuid_data: CpuIdProfileData =
            serde_json::from_slice(include_bytes!("./cpu_profiles/skylake.cpuid.json")).unwrap();

        assert_eq!(cpuid_data, deserialized_cpuid_data);

        let msr_data = profile.msr_data().unwrap();
        let deserialized_msr_data: MsrProfileData =
            serde_json::from_slice(include_bytes!("./cpu_profiles/skylake.msr.json")).unwrap();
        assert_eq!(msr_data, deserialized_msr_data);
    }
}
